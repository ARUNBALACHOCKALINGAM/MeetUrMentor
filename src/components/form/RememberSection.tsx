  ...
  const handleRoleChange = useRoleChange();
    ...
    return (
      <div className="flex items-center justify-between text-sm mt-4 p-2">
        ...
        <Link onClick={()=>{userType==="mentor" ? handleRoleChange("mentor") : handleRoleChange("student")}} to="/forgotpassword" className={`text-sm font-semibold ${userType==="mentor" ? "text-mentorAccent/75 hover:text-mentorAccent" : "text-studentAccent/75 hover:text-studentAccent"} `}>Forgot password?</Link>
      </div>
    );
  };
