...
export const TaskDetails = () => {
        ...
        const fetchTask = async () => {
            try {
                ...
                const formattedResources = taskData.resources.map((resource: any) => ({
                    ...
                    taskId: resource.metadata.taskId
                }));
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const newFiles = files.map((file) => ({
            // Match GridFS file schema structure
            filename: file.name,
            mimetype: file.type,
            size: file.size,
            metadata: {
                taskId: taskId, // From route params
                mimetype: file.type,
                size: file.size
            },
            // Frontend-specific temporary properties
            _id: Math.random().toString(), // Temporary ID for local state management
            url: URL.createObjectURL(file), // Client-side preview URL
            data: file, // Original File object for upload
            uploadDate: new Date() // Mimic backend schema property
        }));
